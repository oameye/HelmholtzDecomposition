...
If[k != m, For[i=1,i<=dim,i++,Assert[ D[f[[k]],xvec[[i]]]===0 || i==k || i==m]]];
...
(* Calculate Fij *)
Fij = Simplify[Table[KroneckerDelta[k,i]*Sum[ (-1)^p/u*D[ LaplacianIncompleteP[W, p, m, xvec],xvec[[j]], {xvec[[m]],2\[Lambda]-2p-2} ],{p,0,\[Lambda]-1}] ,{i,dim},{j,dim}]];
Return[Fij]
